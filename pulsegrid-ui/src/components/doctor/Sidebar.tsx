...
export default function Sidebar() {
      ...
      case "Patient":
        return [
          ...
          },
          {
            ...
            href: "/dashboard/patient/live-monitoring",
            icon: Heart,
          },
          {
            title: "Settings",
  ...
  return (
    <aside className="w-full md:w-[260px] lg:w-[280px] bg-white border-r border-slate-200 md:h-screen md:sticky md:top-0 flex flex-col shrink-0 overflow-hidden">
      {/* Logo */}
      <div className="p-4 sm:p-5 lg:p-6 border-b border-slate-100">
        <PulseGridLogo size="sm" />

        {user?.hospitalName && (
          <p className="text-[10px] text-teal-600 mt-1 uppercase font-semibold tracking-wider break-words">
            {user.hospitalName}
          </p>
        )}
      </div>
        ...
        {menu.map((item) => {
          ...
          return (
            <Link key={item.title} href={item.href}>
              <div
                className={`flex items-center gap-3 px-3 sm:px-4 py-3 rounded-xl cursor-pointer transition-all duration-300 mb-2 ${
                  isActive
      ...
      {/* Profile & Logout */}
      <div className="p-3 sm:p-4 border-t border-slate-100 space-y-3">
        {user && (
          <div className="bg-slate-50 rounded-2xl p-3 sm:p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-r from-teal-500 to-cyan-500 flex items-center justify-center text-white text-xs font-bold shadow-sm flex-shrink-0">
              {getAvatarInitials()}
            </div>
